...
A bottom up (iterative) implementation of the general Viterbi
algorithm in python
...
def viterbiBot(nodes, topo, graph):
  
  paths = 0

  for x, t in enumerate(topo):
    foo = topo[x]
    if foo == 1:
      continue
    if foo == nodes:
      return paths
    #print "min: ", foo
    #print "node ", foo, "has outdegree: ", graph[foo-1]
    paths += graph[foo-1]
    #print "paths: ", paths    
  return paths
